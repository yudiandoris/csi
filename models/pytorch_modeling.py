...
from torch.nn import CrossEntropyLoss, LogSoftmax, KLDivLoss
...
class BertForQuestionAnsweringKL(PreTrainedBertModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringKL, self).__init__(config)
        self.bert = BertModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, start_probs=None, end_probs=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            loss_fct1 = LogSoftmax()
            loss_fct2 = KLDivLoss()
            start_logsoftmax = loss_fct1(start_logits)
            end_logsoftmax = loss_fct1(end_logits)
            start_probs = start_probs
            end_probs = end_probs
            #start_probs = start_probs
            #end_probs = end_probs

            start_loss = loss_fct2(start_logsoftmax, start_probs)
            end_loss = loss_fct2(end_logsoftmax, end_probs)
            total_loss = (start_loss + end_loss) / 2

...
def torch_init_model(model, init_checkpoint, resumepar = False):
    '''
    if resumepar:
        par = torch.load(init_checkpoint)
        assertcnt = len(par.items())

        par = {k[len("module."):]: v for k, v in par.items() if k not in {"module.classifier.weight", "module.classifier.bias", "classifier.weight", "classifier.bias"}}
        #par = {k: v for k, v in par.items() if k not in {"module.classifier.weight", "module.classifier.bias", "classifier.weight", "classifier.bias"}}

        assert(abs(assertcnt - len(par.items())) <= 2)
        #model.load_state_dict(par, strict=False)
        model.load_state_dict(par, strict=False)
     '''

    if resumepar:
        par = torch.load(init_checkpoint, map_location='cpu')
        ...
        state_dict = par.copy()
        state_dict = {k[len("module."):]: v for k, v in par.items() if
        ...
        print('error msgs:{}'.format(error_msgs), flush=True)
